import { DiscordArgumentMetadata, DiscordPipeTransform, ReflectMetadataProvider } from '@discord-nestjs/core';
...
export class TransformPipe implements DiscordPipeTransform {
	...
	transform(interaction: Interaction, metadata: DiscordArgumentMetadata<'interactionCreate'>) {
		...
		Object.keys(originalInstance).forEach((property: string) => {
			const paramDecoratorMetadata = this.metadataProvider.getParamDecoratorMetadata(originalInstance, property);

			if (!paramDecoratorMetadata) return;

			const { name, required } = paramDecoratorMetadata;

			newInstance[property] = interaction.options.get(name ?? property, required)?.value || originalInstance[property];
		});
